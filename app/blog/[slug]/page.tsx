...
export async function generateMetadata({
  ...
  const base =
    process.env.NEXT_PUBLIC_SITE_URL ?? "https://www.pcachurchsydney.com";
...
export default async function Post({
  ...
  const { data: post, error } = await supabase
    .from("blogs")
    .select(
      "id, title, image, author, publish_date, slug, content, excerpt, tags",
    )
    .eq("slug", slug)
    .maybeSingle();
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-[1140px] mx-auto px-4 py-12 sm:px-6 lg:px-8">
        <h1 className="text-4xl font-bold text-church-blue mb-7">{title}</h1>
        ...
        <div className="mb-6">
          {tags.map((tag: string) => (
            <span
              key={tag}
              className="inline-block bg-gray-100 text-sm text-gray-700 mr-2 px-2 py-1 rounded"
            >
              {tag}
            </span>
          ))}
